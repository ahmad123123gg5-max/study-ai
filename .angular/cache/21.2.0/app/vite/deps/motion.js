...
function resolveTransition(transition, parentTransition) {
  if (transition?.inherit && parentTransition) {
    const _a = transition, { inherit: _ } = _a, rest = __objRest(_a, ["inherit"]);
...
function animateTarget(visualElement, targetAndTransition, { delay: delay2 = 0, transitionOverride, type } = {}) {
  let _a = targetAndTransition, { transition, transitionEnd } = _a, target = __objRest(_a, ["transition", "transitionEnd"]);
...
var mask = __spreadProps(__spreadValues({}, complex), {
  getAnimatableNone: (v) => {
    ...
    return transformer(parsed.map((v2) => typeof v2 === "number" ? 0 : typeof v2 === "object" ? __spreadProps(__spreadValues({}, v2), { alpha: 1 }) : v2));
  }
});
...
function springValue(source, options) {
  return followValue(source, __spreadValues({ type: "spring" }, options));
}
function attachSpring(value, source, options) {
  return attachFollow(value, source, __spreadValues({ type: "spring" }, options));
}
...
function getOrCreateRecord(element, parentProjection, projectionOptions) {
  ...
  visualElement.projection.setOptions(__spreadProps(__spreadValues({}, projectionOptions), {
    visualElement
  }));
...
function createAnimationsFromSequence(sequence, _a = {}, scope, generators) {
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    const resolveValueSequence = (valueKeyframes, valueTransition, valueSequence, elementIndex = 0, numSubjects = 0) => {
      ...
      const _a2 = valueTransition, { delay: delay2 = 0, times = defaultOffset(valueKeyframesAsList), type = defaultTransition.type || "keyframes", repeat, repeatType, repeatDelay = 0 } = _a2, remainingTransition = __objRest(_a2, ["delay", "times", "type", "repeat", "repeatType", "repeatDelay"]);
      ...
      if (numKeyframes <= 2 && createGenerator) {
        ...
        const springTransition = __spreadValues(__spreadValues({}, defaultTransition), remainingTransition);
        if (duration !== void 0) {
  ...
  sequences.forEach((valueSequences, element) => {
    for (const key in valueSequences) {
      ...
      const _a2 = defaultTransition, { type: _type } = _a2, remainingDefaultTransition = __objRest(_a2, ["type"]);
      definition.transition[key] = __spreadValues(__spreadProps(__spreadValues({}, remainingDefaultTransition), {
...
function createScopedAnimate(options = {}) {
  ...
  function scopedAnimate(subjectOrSequence, optionsOrKeyframes, options2) {
    ...
    if (isSequence(subjectOrSequence)) {
      animations = animateSequence(subjectOrSequence, reduceMotion !== void 0 ? __spreadValues({ reduceMotion }, optionsOrKeyframes) : optionsOrKeyframes, scope);
    } else {
      const _a = options2 || {}, { onComplete } = _a, rest = __objRest(_a, ["onComplete"]);
      if (typeof onComplete === "function") {
        animationOnComplete = onComplete;
      }
      animations = animateSubject(subjectOrSequence, optionsOrKeyframes, reduceMotion !== void 0 ? __spreadValues({ reduceMotion }, rest) : rest, scope);
    }
...
function scrollInfo(onScroll, _a = {}) {
  var _b = _a, { container = document.scrollingElement, trackContentSize = false } = _b, options = __objRest(_b, ["container", "trackContentSize"]);
  if (!container)
    return noop;
