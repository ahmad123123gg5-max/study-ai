...
import { exportTranslatedFile, streamTranslateFile, translateFile } from './server/file-translator.js';
...
const cookieSameSite = COOKIE_SAME_SITE === 'none' || COOKIE_SAME_SITE === 'strict' || COOKIE_SAME_SITE === 'lax'
    ? COOKIE_SAME_SITE
...
const getRequestedClientOrigin = (req) => {
...
const sendOAuthPopupResult = (res, status, provider, message, targetOrigin = primaryAppUrl) => {
...
app.get('/api/auth/oauth/google/start', (req, res) => {
    ...
    const state = jwt.sign({ provider: 'google', nonce: randomUUID(), origin: clientOrigin }, JWT_SECRET, { expiresIn: '10m' });
...
const googleOAuthCallbackHandler = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const token = createAuthToken(user);
        setAuthCookie(res, token);
        sendOAuthPopupResult(res, 'success', 'google', `Signed in successfully as ${user.email}`, openerOrigin);
    }
    catch (error) {
...
const toClientTranslationResult = (result) => ({
    ...result,
...
app.post('/api/file-translator/translate', asyncHandler(async (req, res) => {
    if (!OPENAI_API_KEY) {
        res.status(500).json({ error: 'OPENAI_API_KEY not configured' });
        return;
    }
...
app.post('/api/file-translator/stream', asyncHandler(async (req, res) => {
    if (!OPENAI_API_KEY) {
        res.status(500).json({ error: 'OPENAI_API_KEY not configured' });
        return;
    }
...
app.get('/api/file-translator/glossary', asyncHandler(async (req, res) => {
...
app.post('/api/file-translator/export', asyncHandler(async (req, res) => {
    const body = (req.body || {});
    if (!body.result) {
