...
import { buildResultCacheKey, buildUnitCacheKey, ensureFileTranslatorStore, findRelevantGlossaryEntries, hashFileBuffer, loadExtractionCache, loadResultCache, loadUnitCache, markGlossaryUsage, saveExtractionCache, saveResultCache, saveUnitCache, upsertGlossaryEntries } from './file-translator-store.js';
...
const normalizeText = (value) => value
    ...
    .replace(/\n{3,}/g, '\n\n')
    .trim();
const isCjkText = (value) => /[\u3400-\u9fff\u3040-\u30ff\uac00-\ud7af]/u.test(value);
const isJoiningScriptText = (value) => /[\u0590-\u08ff]/u.test(value);
const bucketPdfY = (value) => Math.round(value / 2) * 2;
const shouldInsertSpaceBetweenPdfSegments = (previous, current) => {
    if (!previous.text || !current.text)
        return false;
    if (/\s$/u.test(previous.text) || /^\s/u.test(current.text))
        return false;
    if (/[-/([{'"`]\s*$/u.test(previous.text))
        return false;
    if (/^[,.;:!?%)\]}،؛؟]/u.test(current.text))
        return false;
    if (isCjkText(previous.text) || isCjkText(current.text))
        return false;
    ...
    if (gap <= 0.75)
        return false;
    if (isJoiningScriptText(previous.text) &&
...
const joinPdfSegments = (segments) => {
...
const isLikelyPdfListLine = (text) => /^\s*(?:[-*•▪◦‣]|\d+[\.\)]|[A-Za-z][\.\)])\s+/u.test(text);
const isLikelyPdfHeading = (text, fontSize, baselineFontSize) => {
    const trimmed = text.trim();
    if (!trimmed)
        return false;
    if (trimmed.length <= 80 && /^[A-Z0-9\s\-&,/:()]+$/u.test(trimmed) && trimmed.length > 4)
        return true;
    if (fontSize >= baselineFontSize * 1.18 && trimmed.length <= 140)
        return true;
    return false;
};
const classifyPdfBlock = (lines, baselineFontSize, pageWidth) => {
...
const buildPdfRowCells = (segments) => {
    if (!segments.length)
        return [];
    const sorted = [...segments].sort((a, b) => a.x - b.x);
    const cells = [];
...
const buildPdfStructuredRows = (rowsByY, viewportHeight) => rowsByY
    .sort((a, b) => b[0] - a[0])
    .map(([rawY, segments]) => {
...
const clusterPdfColumnAnchors = (rows) => {
    const anchors = [];
...
const mapPdfRowToColumns = (row, columnAnchors, columnCount) => {
...
const groupPdfLinesIntoBlocks = (lines, pageWidth) => {
    if (!lines.length)
        return [];
    ...
    for (const line of lines) {
        ...
        const shouldContinueParagraph = compactGap &&
...
const buildDocumentId = (buffer, fileName) => createHash('sha1').update(fileName).update(buffer).digest('hex').slice(0, 16);
...
const isCompleteTranslationResult = (result) => {
    if (!result)
        return false;
    ...
    if (!groupCount || !unitCount)
        return false;
    if ((result.isComplete ?? true) === false)
        return false;
    if ((result.totalGroups || groupCount) > groupCount)
        return false;
    if ((result.totalUnits || unitCount) > unitCount)
        return false;
    if ((result.completedGroups || groupCount) < (result.totalGroups || groupCount))
        return false;
    return true;
};
...
const assertNotCancelled = (hooks) => {
    if (hooks.isCancelled?.()) {
        throw new TranslationCancelledError();
    }
};
const extractTexts = (xml, tagPattern) => [...xml.matchAll(tagPattern)].map((match) => decodeXml(match[1] || '')).filter((value) => value.trim().length > 0);
const createTextParagraph = (text, rtl, headingLevel) => new Paragraph({
    bidirectional: rtl,
...
const buildTranslationPrompt = (request, priorContext, glossaryLines) => {
    const modeGuide = request.translationMode === 'medical'
...
const summarizeContext = (translatedUnits) => translatedUnits.slice(-5).map((unit) => `${unit.label}: ${unit.translatedText.slice(0, 220)}`).join('\n');
const buildGlossaryLines = (entries) => entries.slice(0, 30).map((entry) => `${entry.source} => ${entry.target}${entry.notes ? ` (${entry.notes})` : ''}`).join('\n');
async function runVisionOcr(dataUrl, request, context) {
...
async function extractPdfPage(pdf, pageNumber, request, context) {
    ...
    const tableRowTops = new Set(detectedTables.flatMap((table) => table.rows.map((row) => Math.round(row.top))));
    ...
    const blockEntries = groupPdfLinesIntoBlocks(lineEntries, viewport.width);
...
async function extractDocx(buffer) {
    ...
    for (const block of blockMatches) {
        ...
        if (isHeading && currentGroupIds.length > 0)
            pushGroup();
        currentGroupIds.push(id);
        currentGroupTexts.push(text);
        if (currentGroupIds.length >= 8)
            pushGroup();
    }
    pushGroup();
...
async function translateChunk(chunk, request, context, completedUnits, glossaryLines) {
    const result = await createOpenAIChatCompletionDetailed({
        apiKey: context.apiKey,
        model: context.model,
...
const mergeGroupTranslation = (group, units) => ({
    ...group,
    translatedText: normalizeText(group.unitIds
        .map((unitId) => units.find((unit) => unit.id === unitId)?.translatedText || '')
        .join('\n\n'))
});
async function streamDocxOrPptxExtraction(buffer, fileType) {
    return fileType === 'docx' ? extractDocx(buffer) : extractPptx(buffer);
}
async function translateGroup(sourceUnits, request, context, glossaryVersion, completedUnits) {
    const relevantGlossary = await findRelevantGlossaryEntries(request.sourceLanguage, request.targetLanguage, request.translationMode, sourceUnits.map((unit) => unit.sourceText).join('\n'));
    ...
    const translateLinearUnits = async (units) => {
        const translated = [];
        const uncached = [];
        let cacheHits = 0;
        for (const unit of units) {
    ...
    const translateTableUnit = async (unit) => {
        if (!unit.tableData) {
        ...
        const translatedTableData = {
            columnCount: unit.tableData.columnCount,
            headers: unit.tableData.headers.map((row, rowIndex) => row.map((cell, columnIndex) => ({
                sourceText: cell.sourceText,
                translatedText: translatedMap.get(`${unit.id}-h-${rowIndex}-${columnIndex}`) || cell.translatedText || ''
            }))),
            rows: unit.tableData.rows.map((row, rowIndex) => row.map((cell, columnIndex) => ({
                sourceText: cell.sourceText,
        ...
        const translatedUnit = {
            ...unit,
            translatedText: normalizeText([...translatedTableData.headers, ...translatedTableData.rows]
...
async function streamTranslationPipeline(request, context, hooks = {}) {
    ...
    const previewGroupLimit = typeof request.previewGroupLimit === 'number' && request.previewGroupLimit > 0
        ...
        : null;
    const persistedGlossary = await upsertGlossaryEntries((request.glossaryEntries || []).map((entry) => ({
        source: entry.source,
        target: entry.target,
        notes: entry.notes,
        sourceLanguage: request.sourceLanguage,
    ...
    const allUnits = [];
    const allWarnings = [];
    const completedUnitsForContext = [];
    const cachedExtraction = await loadExtractionCache(fileHash);
    let completedGroupCount = 0;
    const emitGroupReady = (group, units, translationCached, extractionCached, knownTotalGroups, knownTotalUnits) => {
        ...
        try {
            ...
            for (let pageNumber = 1; pageNumber <= targetPageCount; pageNumber += 1) {
                ...
                for (const warning of extracted.warnings)
                    emitEvent(hooks, { type: 'warning', warning });
...
const buildDocxExport = async (result, bilingual) => {
...
const replacePptxShapeTexts = (slideXml, replacements) => {
...
const buildPptxExport = async (result, bilingual) => {
...
export async function translateFile(request, context) {
    return streamTranslationPipeline(request, context);
}
export async function streamTranslateFile(request, context, hooks) {
    try {
...
export async function exportTranslatedFile(result, format) {
