...
const encodeCommand = (parts: string[]): string =>
  `*${parts.length}\r\n${parts.map((part) => `$${Buffer.byteLength(part)}\r\n${part}\r\n`).join('')}`;

const parseReply = (buffer: Buffer, start = 0): ParsedReply | null => {
...
export class RedisCacheLayer implements HybridCacheLayer {
  readonly name = 'l2_redis' as const;
  ...
  private async execute(commands: string[][]): Promise<RedisReply[]> {
    if (!this.url) {
  ...
  async get<T>(key: string): Promise<CacheEnvelope<T> | null> {
    ...
    try {
      ...
      const entry = JSON.parse(rawValue) as CacheEnvelope<T>;
