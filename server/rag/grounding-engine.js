...
const INSUFFICIENT_KNOWLEDGE_MESSAGE = 'The available knowledge base does not contain enough verified information for this question.';
...
const CREATIVE_HINT_RE = /\b(motivat(?:e|ion)|mood|emotion|quote|poem|story|creative|brainstorm|tagline|slogan|marketing|rewrite|paraphrase|translate)\b/i;
const EDUCATIONAL_HINT_RE = /\b(student|teacher|lesson|chapter|study|tutor|quiz|research|explain|explanation|summar(?:y|ize)|topic|academic|exam|simulation|lab|grounding|medical|nursing|engineering|legal|science|course)\b/i;
const FOLLOW_UP_HINT_RE = /\b(explain|clarify|expand|continue|summarize|summarise|more|again|that|this|these|those|why|how)\b/i;
...
const toHash = (value) => createHash('sha1').update(value).digest('hex');
const normalizeArabic = (value) => value
    ...
    .replace(/ة/g, 'ه')
    .replace(/ـ/g, '');
const normalizeSemanticText = (value) => normalizeArabic(value.toLowerCase())
    .replace(/[`~!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
const tokenize = (value) => normalizeSemanticText(value)
    .split(' ')
    .map((token) => token.trim())
    .filter((token) => token.length > 1)
    .filter((token) => !ENGLISH_STOP_WORDS.has(token))
    .filter((token) => !ARABIC_STOP_WORDS.has(token));
const truncateSnippet = (value, maxChars = MAX_SOURCE_SNIPPET_CHARS) => {
...
const cosineSimilarity = (leftVector, leftNorm, rightVector, rightNorm) => {
...
const overlapRatio = (queryTokens, candidateTokens) => {
...
const chunkLongText = (value, maxChunkLength = 780) => {
...
export class GroundingEngine {
    responseCache = new TTLCache(300, RESPONSE_CACHE_TTL_MS);
    queryCache = new TTLCache(500, QUERY_CACHE_TTL_MS);
    embeddingCache = new TTLCache(900, EMBEDDING_CACHE_TTL_MS);
    documentFrequency = new Map();
    chunkCount;
    chunks;
    constructor(seeds = KNOWLEDGE_BASE_SEEDS) {
    ...
    cacheResponse(key, text, metadata) {
        if (!key || !text.trim()) {
            return;
        }
    ...
    createInsufficientKnowledgeMessage() {
    ...
    shouldEnableGrounding(request, mode, featureHint) {
        if (mode === 'off') {
            return false;
        }
    ...
    shouldShortCircuit(request, mode, featureHint) {
        if (request.jsonMode) {
            return false;
        }
        if (request.attachmentsText?.trim()) {
    ...
    buildAttachmentChunks(attachmentsText) {
    ...
    buildQueryText(message, historyText) {
    ...
    detectDomain(value, featureHint) {
    ...
    scoreChunk(query, chunk, predictedDomain) {
        ...
        return Math.min(1, (semantic * 0.68) +
            (lexical * 0.18) +
            (keywordMatch * 0.08) +
            (titleMatch * 0.05) +
            domainBoost +
    ...
    toGroundingSource(chunk, score, labelNumber) {
        return {
    ...
    buildGroundingPrompt(request, contextBlocks) {
    ...
    buildResponseCacheKey(request, sources) {
