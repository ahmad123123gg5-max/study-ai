...
import type { ClinicalCaseDifficulty, ClinicalLabStatus, GeneratedClinicalCase } from './clinical-records.js';
...
interface AiCasePayload {
  diseaseLabelEn?: string;
  diseaseLabelAr?: string;
  diseaseKey?: string;
  specialtyTrackEn?: string;
  specialtyTrackAr?: string;
  runtimeCategory?: string;
  severity?: string;
  complicationEn?: string;
  complicationAr?: string;
  sourceEn?: string;
  sourceAr?: string;
  patientAge?: number;
  ...
  caseDescriptionAr?: string;
  openingMessageEn?: string;
  openingMessageAr?: string;
  treatmentResponseEn?: string;
  treatmentResponseAr?: string;
  medicalHistoryEn?: string[];
  medicalHistoryAr?: string[];
  learningFocusEn?: string[];
  learningFocusAr?: string[];
  recommendedInvestigationsEn?: string[];
  recommendedInvestigationsAr?: string[];
  vitals?: {
    heartRate?: number;
...
const normalizeLabStatus = (value: unknown): ClinicalLabStatus =>
  value === 'critical' || value === 'watch' || value === 'normal' ? value : 'watch';

const normalizeRuntimeCategory = (value: unknown): GeneratedClinicalCase['runtimeCategory'] => {
...
const parsePayload = (raw: string): AiCasePayload | null => {
  try {
    return JSON.parse(raw) as AiCasePayload;
  } catch {
    ...
    try {
      return JSON.parse(raw.slice(start, end + 1)) as AiCasePayload;
    } catch {
      return null;
    }
  }
...
const pickText = (payload: AiCasePayload, language: 'ar' | 'en', key: keyof AiCasePayload, fallback: string): string => {
...
export const buildAiCasePrompt = (input: AiCaseGenerationInput): Pick<HybridChatRequest, 'message' | 'systemInstruction' | 'jsonMode' | 'knowledgeMode' | 'featureHint' | 'maxTokens'> => {
...
export const mapAiCaseToGeneratedCase = (input: AiCaseGenerationInput, rawText: string): GeneratedClinicalCase => {
