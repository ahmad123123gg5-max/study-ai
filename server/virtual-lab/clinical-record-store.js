...
const asRow = (value) => value;
const asRows = (value) => value;
const toHistory = (row) => row ? normalizeStudentCaseHistoryEntry(row) : null;
const toRecord = (row) => row
    ...
    : null;
const writeHistory = (userId, entry) => {
    upsertHistoryStmt.run(userId, entry.caseId, entry.signature, entry.specialty, entry.disease, entry.difficulty, entry.date);
};
const writeRecord = (userId, record) => {
    upsertRecordStmt.run(userId, record.recordId, record.caseId, record.signature, record.specialty, record.specialtyTrack, record.disease, record.difficulty, record.score, record.status, record.date, record.timeSpentSeconds, record.title, record.caseDescription, record.finalEvaluation, record.educationalAnalysis, stringifyJson(record.mistakes) || '[]', stringifyJson(record.correctDecisions) || '[]', stringifyJson(record.treatmentChoices) || '[]', stringifyJson(record.transcript) || '[]', stringifyJson(record.summary), stringifyJson(record.generatedCase), record.levelTier);
};
...
export const ensureClinicalUserMigrated = (userId, legacyHistory, legacyRecords) => {
    const alreadyMigrated = asRow(selectMigrationStmt.get(userId));
    if (alreadyMigrated?.migratedAt) {
        return false;
    }
    const hasStoredHistory = Number(asRow(countHistoryStmt.get(userId))?.total || 0) > 0;
    const hasStoredRecords = Number(asRow(countRecordsStmt.get(userId))?.total || 0) > 0;
...
export const listClinicalCaseHistory = (userId, limit = 8000) => {
    const rows = asRows(listHistoryStmt.all(userId, Math.max(1, Math.min(limit, 8000))));
    return rows
        .map((row) => toHistory(row))
        .filter((entry) => !!entry);
};
export const hasClinicalCaseHistoryEntry = (userId, caseId, signature) => {
    const row = asRow(findHistoryEntryStmt.get(userId, caseId, signature));
    return !!toHistory(row);
};
export const appendClinicalCaseHistory = (userId, entry) => {
    writeHistory(userId, entry);
    return toHistory(asRow(findHistoryEntryStmt.get(userId, entry.caseId, entry.signature))) || entry;
};
export const findClinicalRecordByCaseId = (userId, caseId) => toRecord(asRow(selectRecordByCaseStmt.get(userId, caseId)));
export const upsertClinicalRecord = (userId, record) => {
    writeRecord(userId, record);
    return findClinicalRecordByCaseId(userId, record.caseId) || record;
};
export const findClinicalRecord = (userId, recordIdOrCaseId) => toRecord(asRow(selectRecordByIdOrCaseStmt.get(userId, recordIdOrCaseId, recordIdOrCaseId)));
export const getClinicalRecordCount = (userId) => Number(asRow(countRecordsStmt.get(userId))?.total || 0);
export const getClinicalProgressStats = (userId) => {
    const statsRow = asRow(aggregateStatsStmt.get(userId)) || {
    ...
    const specialtyBreakdown = asRows(specialtyBreakdownStmt.all(userId)).map((row) => ({
...
export const loadClinicalProgressPage = (userId, cursor, limit) => {
    const safeLimit = Math.max(5, Math.min(limit, 40));
    const cursorRow = cursor
        ? asRow(selectCursorStmt.get(userId, cursor))
        : undefined;
    const rows = asRows(cursorRow
...
export const normalizeStoredGeneratedCase = (value) => normalizeGeneratedClinicalCase(value);
