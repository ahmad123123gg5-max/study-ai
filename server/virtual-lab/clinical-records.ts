...
export type ClinicalCaseDifficulty = 'easy' | 'medium' | 'hard' | 'expert';
export type ClinicalRuntimeCategory = 'respiratory' | 'shock' | 'cardiac' | 'seizure';
export type ClinicalCaseStatus = 'completed' | 'failed';
export type ClinicalLevelTier = 'bronze' | 'silver' | 'gold' | 'platinum';
export type ClinicalAgeGroup = 'neonate' | 'child' | 'adult' | 'older-adult';
...
export interface GeneratedClinicalCase {
  ...
  runtimeCategory: ClinicalRuntimeCategory;
  patientAge: number;
  ageGroup: ClinicalAgeGroup;
...
export interface StudentCaseHistoryEntry {
  caseId: string;
  signature: string;
  specialty: string;
  disease: string;
...
export interface StudentClinicalRecord {
  recordId: string;
  caseId: string;
  signature: string;
  specialty: string;
...
export interface ClinicalProgressStats {
  ...
  levelTier: ClinicalLevelTier;
  recommendedDifficulty: ClinicalCaseDifficulty;
}

export interface ClinicalProgressPage {
  items: StudentClinicalRecord[];
  total: number;
  hasMore: boolean;
...
interface DiseaseTemplate {
  key: string;
  category: ClinicalRuntimeCategory;
  label: LocalizedText;
  tracks: string[];
  chiefComplaint: LocalizedText;
  complications: LocalizedText[];
  sources: LocalizedText[];
  histories: LocalizedText[];
  severities: LocalizedText[];
  responses: LocalizedText[];
  learningFocus: LocalizedText[];
  investigations: LocalizedText[];
}

const DEFAULT_STATS: ClinicalProgressStats = {
...
const TRACK_MATCHERS: Array<{ id: string; keywords: string[]; label: LocalizedText }> = [
  {
    id: 'emergency',
...
const localize = (value: LocalizedText, language: 'ar' | 'en'): string => language === 'ar' ? value.ar : value.en;
...
const normalizeDifficulty = (value: unknown): ClinicalCaseDifficulty =>
  value === 'easy' || value === 'medium' || value === 'hard' || value === 'expert'
    ? value
    : 'medium';

const normalizeLabStatus = (value: unknown): ClinicalLabStatus =>
  value === 'critical' || value === 'watch' || value === 'normal'
    ? value
    : 'normal';

const normalizeAgeGroup = (value: unknown): ClinicalAgeGroup =>
  value === 'neonate' || value === 'child' || value === 'adult' || value === 'older-adult'
    ? value
    : 'adult';

const normalizeStatus = (value: unknown): ClinicalCaseStatus =>
  value === 'failed' ? 'failed' : 'completed';

const normalizeLevelTier = (value: unknown): ClinicalLevelTier =>
  value === 'silver' || value === 'gold' || value === 'platinum'
    ? value
    : 'bronze';
...
const inferTrack = (specialty: string, scenario: string, language: 'ar' | 'en'): string => {
...
const inferTrackId = (specialty: string, scenario: string): string => {
  const haystack = normalizeText(`${specialty} ${scenario}`);
  for (const track of TRACK_MATCHERS) {
    if (track.keywords.some((keyword) => haystack.includes(normalizeText(keyword)))) {
      return track.id;
...
const preferredTemplatesForTrack = (specialty: string, scenario: string): DiseaseTemplate[] => {
...
const ageFromGroup = (group: ClinicalAgeGroup, rand: () => number): number => {
  if (group === 'neonate') return Number((2.4 + rand() * 2.1).toFixed(1));
  if (group === 'child') return Math.round(2 + rand() * 11);
  if (group === 'older-adult') return Math.round(61 + rand() * 24);
  return Math.round(18 + rand() * 42);
};

const sexFromSeed = (rand: () => number): 'male' | 'female' => rand() > 0.5 ? 'male' : 'female';
...
const buildLabs = (
  ...
  const defaults: ClinicalLabVariation[] = [
    {
      id: 'wbc',
...
const buildSignature = (parts: string[]): string =>
  createHash('sha256').update(parts.join('|')).digest('hex');

const resolveAgeGroup = (trackId: string, rand: () => number): ClinicalAgeGroup => {
...
const diseaseOrder = (templates: DiseaseTemplate[], history: StudentCaseHistoryEntry[]): DiseaseTemplate[] => {
...
export const normalizeGeneratedClinicalCase = (value: unknown): GeneratedClinicalCase | null => {
...
export const normalizeStudentCaseHistoryEntry = (value: unknown): StudentCaseHistoryEntry | null => {
  if (!value || typeof value !== 'object') {
    return null;
  }
...
export const normalizeStudentClinicalRecord = (value: unknown): StudentClinicalRecord | null => {
  if (!value || typeof value !== 'object') {
    return null;
  }
...
export const generateClinicalCase = (args: {
  ...
  difficulty: ClinicalCaseDifficulty;
  language: 'ar' | 'en';
  history: StudentCaseHistoryEntry[];
  recordCount: number;
...
export const buildClinicalStats = (records: StudentClinicalRecord[]): ClinicalProgressStats => {
  if (records.length === 0) {
...
export const paginateClinicalRecords = (
  records: StudentClinicalRecord[],
  cursor: string | null,
  limit: number
